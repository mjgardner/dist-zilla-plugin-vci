...
$zilla->build;
ok( $git->rev_parse( '-q', '--verify', 'refs/heads/build/master' ),
    'source repo has the "build/master" branch' )
    or diag $git->branch;
is( $git->log('build/master'), 2, 'one commit on the build/master branch' )
    or diag $git->branch;
...
ok( $git2->rev_parse( '-q', '--verify', 'refs/heads/build/topic/1' ),
    'source repo has the "build/topic/1" branch' )
    or diag $git2->branch;
...
$zilla3->build;
is( $git3->log('build/master'), 4, 'two commits on the build/master branch' )
    or diag $git3->branch;
